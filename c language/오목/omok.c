...
void game_control(void)
{
	...
	do
	{
		...
		{
			if (turn == 0 && matrix[1][(x + 1) / 2][y] == 1) {
				printf("\n검은 돌 위에 흰색 돌을 놓을 수 없습니다.");
				continue;
			}
			else if (turn == 1 && matrix[0][(x + 1) / 2][y] == 1) {
				printf("\n흰색 돌 위에 검은 돌을 놓을 수 없습니다.");
				continue;
			}
			else if (turn == 1 && matrix[1][(x + 1) / 2][y] == 1) {
				printf("\n검은 돌 위에 검은 돌을 놓을 수 없습니다.");
				continue;
			}
			else if (turn == 0 && matrix[0][(x + 1) / 2][y] == 1) {
				printf("\n흰색 돌 위에 흰색 돌을 놓을 수 없습니다.");
				continue;
			}
			else {
